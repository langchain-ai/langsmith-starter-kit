"""Chatbot annotation queues and automations."""
import os
...
def load_automations_and_queues() -> None:
    ...
    queue_name = "Finance QA: Helpfulness Review Queue"
    queue_id = get_queue_id(queue_name)
    if not queue_id:
        queue_id = create_queue(
            name=queue_name,
            description="Queue for manual review of unhelpful chatbot responses",
            instructions="Please review the response and determine if it adequately helps the customer.",
            rubric_items=[{"feedback_key": "helpfulness"}],
        )
    ...
    automation_name = "Helpfulness Review"
    ...
    create_automation(
        ...
        filter='and(eq(is_root, true), and(eq(feedback_key, "helpfulness"), eq(feedback_score, 0)))',
        sampling_rate=1.0,
    )
