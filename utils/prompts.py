"""Shared prompt helpers for LangSmith.

Usage:
    load_prompt("my-prompt", my_chain)
    delete_existing_prompt("my-prompt")
    build_schema(MyPydanticModel, "field_name")

The api_* functions are kept here for advanced use (API-only environments)
but are not needed in the standard workflow.
"""
import json
import requests
...
def delete_existing_prompt(name: str) -> None:
    """Delete a prompt by name (used before re-pushing with multiple commits)."""
    try:
...
# ---------------------------------------------------------------------------
# REST API helpers — for advanced / API-only use
# ---------------------------------------------------------------------------
...
def prompt_exists(prompt_or_ref: str, owner: Optional[str] = None) -> bool:
    """Return True if the prompt repo/commit exists (API-only check)."""
...
def api_push_prompt_commit(name: str, obj: Any, owner: Optional[str] = None) -> Optional[str]:
    """Push a prompt commit via the LangSmith REST API (API-only environments)."""
    resolved_owner = get_owner(owner)
